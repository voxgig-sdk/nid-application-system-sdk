# nid-application-system-cli

AQL-driven command-line client **and** interactive REPL for the NidApplicationSystem
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nid-application-system-cli)
make build

# 2. See usage (words, entities, env vars)
./nid-application-system-cli --help
...
NID_APPLICATION_SYSTEM_BASE=https://api.example.com ./nid-application-system-cli --help

# 6. No arguments -> interactive REPL
./nid-application-system-cli
nid-application-system> /help
nid-application-system> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nid-application-system-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/nid-application-system-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `nid-application-system>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/nid-application-system-cli`. |
...
(`./nid-application-system-cli <expr>`) and interactively (the REPL), and expressions compose the

...
The Lua SDK for the NidApplicationSystem API. Provides an entity-oriented interface using Lua conventions.
...
```lua
-- Create
local created, _ = client:Application(nil):create({ name = "Example" }, nil)
...
local result, err = client:NidApplicationSystem(nil):load(
  { id = "test01" }, nil
)

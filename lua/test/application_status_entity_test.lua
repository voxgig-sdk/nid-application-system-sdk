...
describe("ApplicationStatusEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:ApplicationStatus(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = application_status_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "application_status." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set NIDAPPLICATIONSYSTEM_TEST_APPLICATION_STATUS_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local application_status_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.application_status")))
    local application_status_ref01_data = nil
    if #application_status_ref01_data_raw > 0 then
      application_status_ref01_data = helpers.to_map(application_status_ref01_data_raw[1][2])
    end
...
function application_status_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/application_status/ApplicationStatusTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read application_status test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("NIDAPPLICATIONSYSTEM_TEST_APPLICATION_STATUS_ENTID")

...
// NidApplicationSystem SDK feature test
//
...
class FtCtrl extends NidApplicationSystemControl
{
...
class FtClient
{
    ...
    private NidApplicationSystemUtility $utility;

    public function __construct(NidApplicationSystemUtility $utility, array $options)
    {
    ...
    public function get_utility(): NidApplicationSystemUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NidApplicationSystemContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NidApplicationSystemContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NidApplicationSystemContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NidApplicationSystemConfig::make_feature($name);
    ...
    private static function buildUrl(NidApplicationSystemSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NidApplicationSystemError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NidApplicationSystemResponse($fetched);
        }
    ...
    private function fail(NidApplicationSystemContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NidApplicationSystemContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

...
module NidApplicationSystemUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NidApplicationSystemResponse.new({})
    ...
    if fetch_err
      ...
      response = NidApplicationSystemResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NidApplicationSystemResponse.new(fetched)
    else

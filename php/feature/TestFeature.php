...
// NidApplicationSystem SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class NidApplicationSystemTestFeature extends NidApplicationSystemBaseFeature
{
    ...
    public function init(NidApplicationSystemContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (NidApplicationSystemContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = NidApplicationSystemParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (NidApplicationSystemContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(NidApplicationSystemContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = NidApplicationSystemParam::call($ctx, $k);

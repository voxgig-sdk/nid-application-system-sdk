...
// NidApplicationSystem SDK retry feature
...
class NidApplicationSystemRetryFeature extends NidApplicationSystemBaseFeature
{
    ...
    public function init(NidApplicationSystemContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (NidApplicationSystemContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(NidApplicationSystemContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(NidApplicationSystemContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

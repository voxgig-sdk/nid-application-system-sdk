...
// NidApplicationSystem SDK ApplicationStatus entity
...
class ApplicationStatusEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ApplicationStatusEntity($this->_client, $opts);
    }

    /**
     * @param ApplicationStatus|array $args ApplicationStatus data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = NidApplicationSystemHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ApplicationStatus|array The current ApplicationStatus data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ApplicationStatus fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = NidApplicationSystemHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ApplicationStatus fields).
     */
    ...
    /**
     * Load a single ApplicationStatus.
     *
     * @param ApplicationStatusLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ApplicationStatusLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ApplicationStatus|array The loaded ApplicationStatus as an assoc-array at the
     *   SDK boundary; throws NidApplicationSystemError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = NidApplicationSystemHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }

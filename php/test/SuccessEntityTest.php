...
class SuccessEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set NIDAPPLICATIONSYSTEM_TEST_SUCCESS_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function success_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["success01", "success02", "success03"] as $k) {
    ...
    $entid_env_raw = getenv("NIDAPPLICATIONSYSTEM_TEST_SUCCESS_ENTID");

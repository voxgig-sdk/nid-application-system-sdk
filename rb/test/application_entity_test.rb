...
class ApplicationEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set NIDAPPLICATIONSYSTEM_TEST_APPLICATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def application_basic_setup(extra)
  ...
  entid_env_raw = ENV["NIDAPPLICATIONSYSTEM_TEST_APPLICATION_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "NIDAPPLICATIONSYSTEM_TEST_APPLICATION_ENTID" => idmap,
  ...
  idmap_resolved = Helpers.to_map(
    env["NIDAPPLICATIONSYSTEM_TEST_APPLICATION_ENTID"])

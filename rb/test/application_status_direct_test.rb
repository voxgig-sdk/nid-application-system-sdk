...
class ApplicationStatusDirectTest < Minitest::Test
  def test_direct_load_application_status
    setup = application_status_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-application_status", setup[:live] ? "live" : "unit")
    if _should_skip
...
def application_status_direct_setup(mockres)

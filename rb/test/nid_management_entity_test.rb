...
class NidManagementEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = nid_management_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "nid_management." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set NIDAPPLICATIONSYSTEM_TEST_NID_MANAGEMENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    nid_management_ref01_data = nil
    if nid_management_ref01_data_raw.length > 0
...
def nid_management_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "nid_management", "NidManagementTestData.json")
  ...
  entid_env_raw = ENV["NIDAPPLICATIONSYSTEM_TEST_NID_MANAGEMENT_ENTID"]

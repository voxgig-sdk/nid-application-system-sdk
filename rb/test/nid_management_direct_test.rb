...
class NidManagementDirectTest < Minitest::Test
  def test_direct_load_nid_management
    setup = nid_management_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-nid_management", setup[:live] ? "live" : "unit")
    if _should_skip
...
def nid_management_direct_setup(mockres)

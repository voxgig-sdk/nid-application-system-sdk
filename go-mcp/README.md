# nid-application-system-mcp

[MCP](https://modelcontextprotocol.io) server exposing the NidApplicationSystem SDK as
two agent tools — `nid-application-system_list` and `nid-application-system_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nid-application-system-mcp)
make build

# 2. Provide credentials via the environment
export NID_APPLICATION_SYSTEM_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user nid-application-system \
  -- /absolute/path/to/nid-application-system-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./nid-application-system-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// nid-application-system_list: first page of records
{ "entity": "application" }
{ "entity": "application", "query": { } }

// nid-application-system_load: one record by id
{ "entity": "application_status", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nid-application-system-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user nid-application-system \
     -- "$PWD"/dist/*/nid-application-system-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `nid-application-system_list` and `nid-application-system_load` tools now appear
   in new sessions. Ask the agent to *"list application using nid-application-system"*
   and it calls `nid-application-system_list` with `{"entity":"application"}`.
...
```sh
export NID_APPLICATION_SYSTEM_APIKEY=sk_live_xxx            # API key
export NID_APPLICATION_SYSTEM_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `nid-application-system_list` tool
...
### Call the `nid-application-system_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `nid-application-system_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `nid-application-system_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NID_APPLICATION_SYSTEM_APIKEY` | API key sent with every request. |
| `NID_APPLICATION_SYSTEM_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

application | application_status | login | nid_management | registration | success

### Smoke test via HTTP (raw JSON-RPC)

```sh
./nid-application-system-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"nid-application-system_load","arguments":{"entity":"application_status","query":{"id":1}}}}'
```

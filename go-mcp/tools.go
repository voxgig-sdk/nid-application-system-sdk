...
type Args struct {
	Entity string         `json:"entity" jsonschema:"application | application_status | login | nid_management | registration | success"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.NidApplicationSystemSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "nid-application-system_list",
		Description: "List records from NidApplicationSystem. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "nid-application-system_load",
		Description: "Load a single record from NidApplicationSystem. " +
...
func runOp(client *sdk.NidApplicationSystemSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.NidApplicationSystemSDK, name string) (sdk.NidApplicationSystemEntity, error) {
	switch strings.ToLower(name) {
	case "application":
		return client.Application(nil), nil
	case "application_status":
		return client.ApplicationStatus(nil), nil
	case "login":
		return client.Login(nil), nil
	case "nid_management":
		return client.NidManagement(nil), nil
	case "registration":

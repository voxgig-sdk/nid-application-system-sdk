...
class TestApplicationStatusDirect:

    def test_should_direct_load_application_status(self):
        setup = _application_status_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-application_status", "live" if setup["live"] else "unit")
        if _skip:
...
def _application_status_direct_setup(mockres):

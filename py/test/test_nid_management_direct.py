...
class TestNidManagementDirect:

    def test_should_direct_load_nid_management(self):
        setup = _nid_management_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-nid_management", "live" if setup["live"] else "unit")
        if _skip:
...
def _nid_management_direct_setup(mockres):

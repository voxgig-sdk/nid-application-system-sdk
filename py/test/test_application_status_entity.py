...
class TestApplicationStatusEntity:

    def test_should_create_instance(self):
        testsdk = NidApplicationSystemSDK.test(None, None)
        ent = testsdk.ApplicationStatus(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _application_status_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "application_status." + _op, "live" if _live else "unit")
            if _skip:
        ...
        application_status_ref01_data = None
        if len(application_status_ref01_data_raw) > 0:
        ...
        application_status_ref01_data_dt0_loaded, err = application_status_ref01_ent.load(application_status_ref01_match_dt0, None)
        assert err is None
        assert application_status_ref01_data_dt0_loaded is not None



def _application_status_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/application_status/ApplicationStatusTestData.json")

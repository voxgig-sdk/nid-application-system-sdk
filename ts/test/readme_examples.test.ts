...
//     (\`new NidApplicationSystemSDK(...)\` / \`NidApplicationSystemSDK.test(...)\` become
//     \`NidApplicationSystemSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'NidApplicationSystemSDK'
...
const TEST_SEED = {"entity":{"application":{"test01":{"id":"test01"}},"application_status":{"test01":{"id":"test01"}},"login":{"test01":{"id":"test01"}},"nid_management":{"test01":{"id":"test01"}},"registration":{"test01":{"id":"test01"}},"success":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NidApplicationSystemSDK(...)\` and any
// \`NidApplicationSystemSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NidApplicationSystemSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NidApplicationSystemSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

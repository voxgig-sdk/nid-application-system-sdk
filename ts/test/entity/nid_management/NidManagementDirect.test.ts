...
describe('NidManagementDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new NidApplicationSystemSDK({
      system: { fetch: async () => ({}) }
    })
  ...
  test('direct-load-nid_management', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-nid_management', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['NIDAPPLICATIONSYSTEM_TEST_NID_MANAGEMENT_ENTID']

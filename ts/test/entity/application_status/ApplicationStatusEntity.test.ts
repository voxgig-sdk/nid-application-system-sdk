...
describe('ApplicationStatusEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'application_status.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set NID_APPLICATION_SYSTEM_TEST_APPLICATION_STATUS_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['NID_APPLICATION_SYSTEM_TEST_APPLICATION_STATUS_ENTID']
